    ...
    <div style={{
      ...
      <motion.div {...fadeUp} transition={{ delay: 0 }}>
        ...
        <p style={{
          ...
          Confidence-Weighted Cross-document Natural Language Inference — an adaptive
          verification framework that adjusts its scrutiny based on evidence quality.
        </p>
      </motion.div>
      ...
      >
        ...
        <div style={{
          ...
          {[
            {
              name: 'Tiered',
              formula: 'T = rs < pivot ? T_strict : T_lenient',
              desc: 'Binary step function. Simple and interpretable — a hard switch at the pivot point. Discontinuous but predictable.',
              pros: 'Easy to tune, very fast',
              cons: 'Discontinuity at pivot',
            },
            {
              name: 'Square Root',
              formula: 'T = T_s - (T_s - T_l) × √rs',
              desc: 'Concave curve that drops the threshold quickly for moderate retrieval scores, then flattens. Smooth and continuous.',
              pros: 'Smooth transition, 2 params',
              cons: 'Sensitive to low retrieval',
            },
            {
              name: 'Sigmoid',
              formula: 'T = T_l + (T_s - T_l) / (1 + e^(k(rs-p)))',
              desc: 'S-shaped logistic transition centered at the pivot. The steepness parameter k controls how sharp the transition is.',
              pros: 'Most flexible, tunable',
              cons: 'Larger hyperparameter space',
            },
          ].map((variant, i) => (
            <motion.div
              key={variant.name}
